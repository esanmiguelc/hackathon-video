...
export default class HighSchoolsComponent extends React.Component {
  ...
  render() {
    ...
    var highSchools = keys.map((key) => {
      ...
      var selectedClass = this.props.selectedId == school.id ? 'selected' : ''
      return (
        <li className={selectedClass} onClick={this.onSchoolClick.bind(this)} data-id={school.id}>{school.name} ({school.count})</li>
      )
    })
    return(
      <ul>
